...
const EventsMonth = () => {
		...
		if (userId !== null) {
			const fetchEvents = async () => {
				try {
					...
					const filteredSeries = filteredLabels.map((key) => eventTypeCounts[key]);
	...
	const options = {
		...
		labels: labels,
		colors: ["#fb923c", "#3b82f6", "#fde047"], // orange-500, blue-500, yellow-400
		legend: {
			position: "bottom",
		},
		dataLabels: {
	...
	return (
		<section aria-label="Events Donut Chart" className="flex flex-col items-center justify-center w-full">
			<Chart
				options={options}
				series={series}
				type="donut"
			...
			/>
			<figcaption className="mt-2 text-sm text-gray-600" id="events-donut-caption">
				{totalEvents} événements au total en {now.toLocaleString("fr", { month: "long", year: "numeric" })}
			</figcaption>
		</section>
	);
};
