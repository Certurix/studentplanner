  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        console.log("[PlanningPerso] Fetching events from:", apiUrl);
        ...
        if (isMounted) {
          ...
          if (!isValidResponse) {
            console.warn("[PlanningPerso] Invalid response format:", response);
          }
        }
      } catch (error) {
        console.error("[PlanningPerso] Error fetching events:", error);
        
        // Log more detailed error information
        if (error.response) {
          console.error(
            "[PlanningPerso] API error response:", 
            error.response.status,
            error.response.statusText
          );
        } else if (error.request) {
          console.error("[PlanningPerso] No response received from API:", error.message);
        }
