  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const apiUrl = `${baseUrl}/api/events/${userId}/month/${currentMonth}?type=2`;
        
        console.log("[PlanningScolaire] Fetching events from:", apiUrl);
        ...
        if (isMounted) {
          ...
          if (!isValidResponse) {
            console.warn("[PlanningScolaire] Invalid response format:", response);
          }
        }
      } catch (error) {
        console.error("[PlanningScolaire] Error fetching events:", error);
        
        // Log more detailed error information
        if (error.response) {
          console.error(
            "[PlanningScolaire] API error response:", 
            error.response.status,
            error.response.statusText
          );
        } else if (error.request) {
          console.error("[PlanningScolaire] No response received from API:", error.message);
        }
